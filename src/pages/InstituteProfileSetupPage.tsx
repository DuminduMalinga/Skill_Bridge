...
const TYPES = ["University", "Training Center", "Bootcamp", "Professional Body"];
const FOCUS_OPTIONS = ["IT", "Engineering", "Business", "Design", "Data Science"];

type SubmitState = "idle" | "saving" | "verified";

export default function InstituteProfileSetupPage() {
  const navigate = useNavigate();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [name, setName] = useState("Riverside Institute of Technology");
  const [type, setType] = useState(TYPES[1]);
  const [location, setLocation] = useState("London, UK");
  const [website, setWebsite] = useState("riversidetech.ac");
  const [tagline, setTagline] = useState("Delivering accredited, industry-aligned technology and business programs that bridge the gap between classroom and career.");
  const [focusAreas, setFocusAreas] = useState<string[]>(["IT", "Business"]);
  const [logoPreview, setLogoPreview] = useState<string | null>(null);
  const [submitState, setSubmitState] = useState<SubmitState>("idle");

  const initial = name.trim().length > 0 ? name.trim().charAt(0).toUpperCase() : "I";

  const score = Math.min(
    100,
    20 +
      (name.trim().length > 2 ? 15 : 0) +
      (type ? 10 : 0) +
      (location.trim().length > 2 ? 10 : 0) +
      (website.trim().length > 3 ? 15 : 0) +
      (tagline.trim().length >= 10 ? 15 : 0) +
      (focusAreas.length > 0 ? 15 : 0)
  );
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => navigate("/institute-dashboard"), 900);
    }, 900);
  }
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-xl shadow-[0_1px_8px_rgba(0,0,0,0.04)]">
        <div className="h-16 max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
          <div className="flex items-center gap-space-sm">
            ...
            <span className="hidden sm:inline-block font-label-sm text-label-sm text-secondary tracking-wide uppercase">Institute Setup</span>
          </div>
            ...
            <div className="w-full max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-space-xl items-start">
              <section className="lg:col-span-7 flex flex-col gap-space-lg bg-surface-container-lowest p-space-lg sm:p-space-xl rounded-xl shadow-sm">
                <header className="flex flex-col gap-space-xs">
                  ...
                  <h1 className="font-headline-lg text-headline-lg text-on-surface">Set up your institute profile</h1>
                  <p className="font-body-md text-body-md text-secondary leading-relaxed">
                    Tell prospective students about your programs, focus areas, and accreditation so they can find the right course, faster.
                  </p>
                </header>
                <form className="flex flex-col gap-space-md" onSubmit={handleSubmit}>
                  <div className="flex flex-col gap-1.5">
                    <label className="font-label-md text-label-md text-on-surface flex items-center justify-between" htmlFor="instituteNameInput">
                      <span>Institute Name <span className="text-error">*</span></span>
                      <span className="font-caption text-caption text-secondary">Public to all students</span>
                    </label>
                    <div className="relative flex items-center">
                      <span className="absolute left-3.5 material-symbols-outlined text-outline text-[20px] pointer-events-none">account_balance</span>
                      <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-space-md">
                    <div className="flex flex-col gap-1.5">
                      <label className="font-label-md text-label-md text-on-surface" htmlFor="instituteTypeSelect">Institute Type <span className="text-error">*</span></label>
                      <div className="relative flex items-center">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="font-label-md text-label-md text-on-surface" htmlFor="locationInput">Location <span className="text-error">*</span></label>
                      <div className="relative flex items-center">
                        <span className="absolute left-3.5 material-symbols-outlined text-outline text-[20px] pointer-events-none">location_on</span>
                        <input
                          className="w-full pl-10 pr-4 py-2.5 bg-surface-container-lowest text-on-surface rounded-lg shadow-sm placeholder:text-outline font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-primary-container/30 transition-all"
                          id="locationInput"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-label-md text-label-md text-on-surface" htmlFor="websiteInput">Institute Website <span className="text-error">*</span></label>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <label className="font-label-md text-label-md text-on-surface">Institute Logo</label>
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col text-center sm:text-left gap-1">
                        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                          <span className="font-label-md text-label-md text-on-surface font-semibold">riverside-institute-mark.svg</span>
                          <span className="px-2 py-0.5 rounded-full bg-secondary-container text-on-secondary-container font-caption text-caption">61 KB</span>
                        </div>
                        <p className="font-caption text-caption text-secondary">
                          Drag and drop a new logo here, or <button className="text-primary font-medium hover:underline inline" type="button">browse computer</button>
                        </p>
                      </div>
                      <button
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-label-md text-label-md text-on-surface">Focus Areas / Departments <span className="text-error">*</span></label>
                    <p className="font-caption text-caption text-secondary -mt-0.5">Select every area your institute offers programs in.</p>
                    <div className="flex flex-wrap gap-2">
                      {FOCUS_OPTIONS.map((area) => {
                        ...
                        return (
                          <button
                            key={area}
                            className={`px-3 py-1.5 rounded-full font-label-sm text-label-sm font-medium transition-colors ${
                              active ? "bg-primary-container text-on-primary" : "bg-surface-container-low text-on-surface-variant hover:bg-surface-container"
                            }`}
                            type="button"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <label className="font-label-md text-label-md text-on-surface" htmlFor="taglineInput">Institute Description / Bio <span className="text-error">*</span></label>
                      <span className="font-caption text-caption text-secondary">{tagline.length} / 160</span>
                    </div>
                    <textarea
                      ...
                      id="taglineInput"
                      maxLength={160}
                      placeholder="e.g. A hands-on training center delivering accredited, industry-aligned technology programs since 2014."
                      required
                      rows={2}
                      value={tagline}
                  ...
                  <div className="flex items-start gap-space-sm p-3.5 rounded-lg bg-surface-container-low">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="font-label-sm text-label-sm text-on-surface font-semibold">Automatic Accreditation Check</span>
                      <p className="font-caption text-caption text-secondary leading-normal">
                        Based on your registered institutional domain (<span className="font-medium text-on-surface">@riversidetech.ac</span>), the{" "}
                        <span className="text-tertiary-container font-medium">SkillBridge Verified Institute Partner</span> seal will automatically activate upon submission.
                      </p>
                    </div>
                  </div>
                  <div className="pt-space-sm flex flex-col sm:flex-row items-center gap-space-md">
                    ...
                    >
                      {submitState === "idle" && (
                        <>
                          <span>Continue to Institute Dashboard</span>
                    ...
                    <span className="font-caption text-caption text-secondary text-center sm:text-left">
                      You can modify all institute specs anytime under <a className="text-primary hover:underline font-medium" href="#">Organization Settings</a>.
                    </span>
                  </div>
                </form>
              </section>
              ...
              <aside className="lg:col-span-5 flex flex-col gap-space-md sticky top-24">
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-label-sm text-label-sm text-on-surface-variant font-semibold tracking-wide uppercase">Live Student Preview</span>
                  </div>
                  <span className="font-caption text-caption text-secondary">Institute Directory Card</span>
                </div>
                <div className="w-full bg-surface-container-lowest rounded-xl p-space-lg shadow-md flex flex-col gap-space-md transition-all duration-300">
                    ...
                    <div className="flex items-center gap-space-sm">
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1.5">
                          <span className="font-headline-sm text-headline-sm text-on-surface font-semibold tracking-tight">{name.trim() || "Institute Name"}</span>
                          <span className="inline-flex items-center text-tertiary-container" title="Verified SkillBridge Institute Partner">
                            <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
                          </span>
                        </div>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="font-caption text-caption text-primary font-medium hover:underline">{website.trim() || "institute.edu"}</span>
                          <span className="text-outline-variant font-caption text-caption">•</span>
                          <span className="font-caption text-caption text-secondary">{location.trim() || "Location"}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="px-2.5 py-1 rounded-full bg-surface-container-low text-on-surface-variant font-caption text-caption font-medium">{type}</span>
                  ...
                  <p className="font-body-md text-body-md text-secondary leading-relaxed line-clamp-3">
                    {tagline.trim() || "Tell students what your institute stands for, the programs you offer, and what they will learn here."}
                  </p>
                  <div className="p-3 bg-surface-container-low rounded-lg flex flex-col gap-2">
                    <div className="flex items-center justify-between text-caption font-caption">
                      <span className="text-secondary font-medium">Focus Areas</span>
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                      {focusAreas.length === 0 ? (
                        <span className="font-caption text-caption text-secondary">No focus areas selected yet</span>
                      ) : (
                        focusAreas.map((area) => (
                          <span key={area} className="px-2 py-0.5 rounded-full bg-surface-container-lowest text-on-surface font-caption text-caption">{area}</span>
                        ))
                      )}
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-space-xs text-secondary font-caption text-caption">
                    <div className="flex items-center gap-1.5">
                      <span className="material-symbols-outlined text-[16px] text-tertiary-container">school</span>
                      <span>Courses launching on submission</span>
                    </div>
                ...
                <div className="p-space-md rounded-xl bg-surface-container-lowest shadow-sm flex items-start gap-space-sm">
                  ...
                  <div className="flex flex-col gap-1">
                    <h2 className="font-label-md text-label-md text-on-surface font-semibold">2.8× Enrollment Velocity</h2>
                    <p className="font-caption text-caption text-secondary leading-relaxed">
                      Institutes with a verified focus and clear program bio receive over double the qualified student enrollment inquiries within their first 14 days.
                    </p>
                  </div>
                </div>
                <div className="p-space-md rounded-xl bg-surface-container-lowest shadow-sm flex flex-col gap-2">
                  <span className="font-label-sm text-label-sm text-on-surface font-semibold flex items-center gap-1.5">
                    ...
                    Pro-Tip for Program Admins
                  </span>
                  <p className="font-caption text-caption text-secondary leading-relaxed">
                    Students frequently filter courses by focus area and format. You&apos;ll be able to tag skills covered, duration, and funding type right on the next screen when you post your first course.
                  </p>
                </div>
              </aside>
            </div>
